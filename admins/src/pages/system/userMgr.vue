<template>
    <div>
      <Row style="border-bottom:2px solid #EEE; padding-bottom: 10px;">
        <Col span="18">
        <div class="cpageTitle"><Icon size="20" type="md-people" /><span>用户统计({{rtotal}})</span></div>
   ...
   <br>
      ...
      <div v-if="active == 1">
        <Row class="rheader">
          <Col span="8" class="tleft">昵称</Col>
          <Col span="4"  class="tcenter">姓名</Col>
          <Col span="4" class="tcenter">手机</Col>
          <Col span="4" class="tcenter">注册时间</Col>
            <Col span="4" class="tcenter">邀请人</Col>
        </Row>
        <Row class="rerow" v-for="(o,idx) in odrList" :key="idx">
            <Col span="8" class="tleft">{{o.nickname}}</Col>
            <Col span="4"  class="tcenter">{{o.name?o.name:'未完善'}}</Col>
            <Col span="4" class="tcenter">{{o.mobile?o.mobile:'未完善'}}</Col>
            <Col span="4" class="tcenter">{{o.create_time}}</Col>
            <Col span="4" class="tcenter">{{o.pname}}</Col>
        </Row>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getused(){
        this.$http.post('member/lists?page='+this.pnumb+"&size="+this.psize,{
