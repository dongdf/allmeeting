<template>
  <div>
    <Row style="border-bottom:2px solid #EEE; padding-bottom: 10px;">
      <Col span="18">
      <div class="cpageTitle"><Icon size="20" type="ios-document" /><span>报告查看</span></div>
      ...
      <span slot="append">
                <Button type="primary" @click="searchcom">搜索</Button>
            </span>
      </Input>
      </Col>
    </Row>
    <Row class="iv_row" v-for="(r,idx) in repList" :key="idx">
      ...
      <Row type="flex" class="yearc" justify="center">
        ...
        <div class="yearc_c">
          ...
          <p class="">
            ...
            <Badge status="success" v-show="r.zh_pf<=3" text="一级" />
            <Badge status="warning" v-show="r.zh_pf>3 && r.zh_pf<=7" text="二级" />
            <Badge status="error" v-show="r.zh_pf>7" text="三级" />
          </p>
      ...
      <Col span="6">
      <div class="edits">
        <Button type="primary" size="small" ghost  @click="godetail(r)">报告详情</Button>
        <Button type="primary"  size="small" ghost   @click="gosimpleDetail(r)">简要报告</Button>
        <Button type="primary"  size="small" ghost   @click="gozjDetail(r)">总结报告</Button>
        <Button type="primary" size="small" ghost  @click="godata(r)">数据源</Button>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      godata(obj){
        let that =this;
        // that.$router.push({name:'gradeAdmin',query:{edit:1,id:obj.qy_id,cur:1}})
        window.open(FLODER+'#/bbview/uploadData?edit=1&id='+obj.qy_id+'&cur=1');
      },
      gozjDetail(obj){
        ...
        if(obj.simple_summary_url){
          ...
          this.$http.get('financial/reportService/result2Html?type=2&year='+obj.nf+'&companyId='+obj.qy_id).then(res=>{
            if(res.data.code!=1){
      ...
      godetail(obj){
        ...
        if(obj.pc_url){
          window.open(obj.pc_url)
        }else{
          this.$http.get('financial/reportService/result2Html?type=0&year='+obj.nf+'&companyId='+obj.qy_id).then(res=>{
            if(res.data.code!=1){
              this.$Message.error(res.data.msg);
            }else{
              var idx = this.repList.findIndex(function (item) {
              ...
              this.repList[idx].pc_url = res.data.data;
              // alert(this.repList[idx].pc_url);
              window.open(res.data.data)
            }
          })
      ...
      gosimpleDetail(obj){
        if(obj.simple_url){
          ...
          this.$http.get('financial/reportService/result2Html?type=1&year='+obj.nf+'&companyId='+obj.qy_id).then(res=>{
            if(res.data.code!=1){
              this.$Message.error(res.data.msg);
            }else{
              var idx = this.repList.findIndex(function (item) {
